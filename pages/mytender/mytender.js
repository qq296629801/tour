...
Page({
  ...
  setList: function () {
    let that = this;
    var data = wx.getStorageSync('tender');
    console.log(data);
    if (data != null) {
      this.setData(data)
    }
    api.get(api.SUPPLIER + "/tender", { supplierId: 1, page:1 })
      .then(res => {
  ...
  searchScrollLower: function () {
    ...
    if (page <= that.data.totalPage) {
      api.get(api.PURCHASER + "/list", { supplierId: 1, page: page })
        .then(res => {
