...
Page({
  ...
  setList: function () {
    let that = this;
    var data = wx.getStorageSync('product');
    
    if (data != null) {
      this.setData(data)
    }
    api.get(api.SUPPLIER + "/product", { supplierId: 1, page: 1 })
      .then(res => {
        var datas = res.data.data.list;
        for (var i in datas) {
          datas[i].columns.create_date = util.formatTime(datas[i].columns.create_date)
        }
        console.log(datas);
  ...
  searchScrollLower: function () {
    ...
    if (page <= that.data.totalPage) {
      api.get(api.SUPPLIER + "/product", { supplierId: 1, page: page })
        .then(res => {
