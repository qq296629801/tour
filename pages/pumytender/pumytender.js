...
Page({
  ...
  setList: function (page,id) {
    let that = this;
    var data = wx.getStorageSync('putenderlist');
    console.log(data);
    if (data != null) {
      this.setData(data)
    }
    api.get(api.PURCHASER + "/list", { purchaser_id: id,page,page})
      .then(res => {
        ...
        wx.setStorageSync('putenderlist', {
          activitylist: datas,
  ...
  searchScrollLower: function () {
    ...
    if (page <= that.data.totalPage) {
      api.get(api.PURCHASER + "/list", { page: page, purchaser_id:1})
        .then(res => {
